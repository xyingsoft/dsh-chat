...
> 已接 `/api/chat/conversations` 与 `/api/chat/messages/history`，界面显示的是数据库里真实的消息。编辑与撤回如实呈现：撤回后**服务端**把正文换成占位，原文不出现在响应里。截图见 [DSH 装载验证 · 验证六](./docs/_meta/dsh-integration-evidence.md)。
>
