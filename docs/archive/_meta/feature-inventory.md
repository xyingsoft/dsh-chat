...
| 身份 | 开户界面 | ✅ |
| 身份 | 注销（远端撤销 + 本地清除） | ✅ |
| 身份 | 请求签名接入请求路径（host → relay） | ✅ |
| 身份 | relay 指纹带外钉住（防中间人） | ✅ |
| 身份 | `TIME_SKEW` 自动校准并重试 | ✅ |
| 组织 | 列出全组织成员（需 `organization.manage`） | ✅ |
| 组织 | 改成员角色（乐观并发） | ✅ |
| 组织 | 移除成员（状态变更，不删行） | ✅ |
| 组织 | 最后一个所有者不可降级/移除 | ✅ |
| 投递 | SSE 端点注册并推送 | ✅ |
| 投递 | 客户端订阅事件流，断开显式呈现 | ✅ |
| 在线 | 心跳、聚合状态、时钟异常判 `unknown` | ✅ |
| 在线 | 可见性三档（服务端过滤） | ✅ |
| 在线 | 会话列表状态点（形状 + 文本，不只靠颜色） | ✅ |
| 在线 | relay 侧心跳与查询（跨机器） | ✅ |
| 在线 | 可见性读写端点与抽屉里的选择器 | ✅ |
| 在线 | `shared_scopes` 真查成员关系（两侧） | ✅ |
...
| ~~请求签名未挂进请求路径~~ | **已完成**（host → relay 那一跳）。relay 侧配 TLS 指纹即启用，host 侧自动签名并处理 `TIME_SKEW` 校准；指纹可带外钉住防中间人 |
| **browser → host 这一跳没有签名** | 同机同源，跨源写请求已被挡掉，暴露面比远端那一跳小。但别把「远端签了」当成「整条链路签了」 |
| **relay 不配指纹时签名不启用** | 进程只听明文 HTTP，TLS 在反向代理终止，指纹拿不到只能配。不配时启动打明确警告 |
| **在线状态的领域代码两侧各一份** | 仓库拆分的既定代价。contract 靠 vendored 校验防漂移，领域代码没有那层保护 —— 改折叠规则要两边的测试都跑 |
...
| ~~SSE 未接到界面~~ | **已完成**。端点此前**根本没在插件里注册过**；现在发消息会推给收件人与发送者的其他设备，界面订阅并把断开如实显示（§4） |
| ~~组织与成员的部分端点缺失~~ | **已完成**。补了列名单、改角色、移除，含「最后一个所有者不能被降级或移除」的护栏 |
| **测试偶发 worker 崩溃** | 全量 `vitest run` 约每 12 次一次，改动前即存在 |
| **一次未复现的偶发断言失败** | 2026-08-30 全量跑时 `message-edit.host.spec.ts > 通用边界 > 不存在的消息返回 404，与无权限不可区分` 失败一次，随后 20 次连跑未复现。**与上一条不是同一个症状**（那条是 worker 崩溃，这条是断言失败），单列以便下次出现时有线索。该用例的时间是固定的 `NOW`，库是 `:memory:`，端口是 0 —— 常见的三类不稳定源都排除了 |
...
~~另有两条 §50 开放决策标注为 **(P0)** 但尚未关闭~~ —— **已于 2026-08-30 关闭**：
第二因素在 P0 对组织所有者强制（`webauthn` 保持 P4）；在线状态取心跳 30 秒 /
`online` 90 秒 / `idle` 10 分钟，按项目订阅不在 P0 做。两条都是**按默认取值
关闭**的，理由与可推翻性写在 §50 各自的条目下。
