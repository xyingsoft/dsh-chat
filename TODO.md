...
最后更新：2026-08-30 · 20 个 PR 已合并 · 190 个测试
...
**尚不可用**：界面（slot 注册被类型依赖挡住）、第二因素与恢复、在线状态、本地搜索、协议版本协商、群聊、附件。这些入口按文档要求显式缺失，**不伪装为可用**。
...
| 品牌化标识符与持久化类型 | ✅ |
| 命令与事件的 schema 定义 | ⬜ |
| `AuditEvent` 的契约层定义（当前在 audit 包内） | ⬜ |
| 协议版本协商的编解码 | ⬜ |
...
| 缺口 | 影响 | 处置 |
|---|---|---|
| `settings.section` 的 slot 类型不可得 | 界面无法注册 | 见阶段 10 |
