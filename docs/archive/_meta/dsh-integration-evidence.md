...
会话列表带未读角标与「你：」前缀，消息视图分左右气泡，编辑过的那条标注
「已编辑」，撤回的显示为占位且**原文不出现在响应里**。当时的截图后来被验证七、八
的取代（面板换了位置），最终形态见下方。
...
## 验证七、八：面板位置与可交互（2026-08-30）

界面此前落在设置分区里 —— 那是将就，也偏离了 §5：文档明确把「会话列表」与
「消息视图」列为**各自独立的 slot 贡献**。而且它当时**只能读不能写**，没有输入框。

现在挂在 `conversation.session.header.utilities`，点会话头部右上角的「聊天」
从右侧滑出可调宽度的抽屉，带输入框：
...
```
最近 3 条消息：
  01a0510b jia -> yi | 这条是从 DSH 界面发出去的
未读（未 ACK 的队列项）：
  jia -> 4
审计事件数： 2
```

三件事一次验证到：消息 ID 是 **UUIDv7**（`01a0510b` 前缀就是毫秒时间戳，§14
要求客户端生成 v7 而不是 `crypto.randomUUID()` 给的 v4）；打开会话时的**标记已读
生效**；**审计与领域写入同事务**。

### 两处只有真机才暴露的问题

**抽屉头部被标题栏盖住。** 第一版写 `top: 0`，而 DSH 有自绘标题栏 —— 抽屉钻到
它下面，标题与关闭按钮整个看不见。改为**量**触发按钮的下边缘。

**内容只占上面一小块。** `ChatSection` 原来写死 `height: 420px`。改成填满容器，
高度约束交给外层。

### 一处存储键的坑

改默认宽度（380 → 520）后界面没变化：旧值存在 `localStorage` 里，在新区间内仍然
合法，于是被原样沿用。键名加 `:v2` 后缀让旧值失效一次。**默认值变了而存储键没变，
等于改了个寂寞。**

### 试过但退回的方案：「聊天」标签页

为解决「一个页面两个输入框」，把聊天做成了 `conversation.view` 的第三个标签，
指望切过去就只剩一个输入框。**做出来了，然后退回了** —— 官方的「轨迹」标签同样
保留原生输入框：`conversation.view` 只替换正文区，输入框由 `ConversationRoot`
常驻渲染，没有 opt-out。结果是两个输入框**上下紧挨着**，比抽屉更糟。

抽屉至少把它们在空间上分开：我们的在右侧栏内，原生的在主区域左下。
这不是理想解，理想解见[功能全量清单的「已确认做不到的事」](./feature-inventory.md#八已确认做不到的事)。
