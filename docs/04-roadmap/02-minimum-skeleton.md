...
| | |
|---|---|
| **回答什么问题** | P0 何时才算完成：两台 DSH Web 实例加一个持久化 relay 必须跑通哪 14 步，以及这 14 步如何拆进两个验收关口。 |
| **谁该读** | 项目负责人，架构师，各插件包实现者，验收执行人 |
| **前置阅读** | [整体架构](../02-architecture/01-overall-architecture.md)，[插件化架构 §6.2 bundle 的装载形态](../02-architecture/02-plugin-model.md#62-bundle-的装载形态) |

**关键约束**（最易被违反的三条，不能替代正文）：

1. 关口顺序不可颠倒：`P0-a` 九步未全绿不得进入 `P0-b`，只完成 `P0-a` 也不得对外宣布 P0 交付或进入 P1。
2. 两个关口合计仍是完整的 14 步，范围没有缩减；不得为压缩前一关口而删除 schema 中的恢复字段或协议协商字段。
3. P0 未装载的包必须显式返回 `NOT_IMPLEMENTED`，不得以空实现或假界面伪装为可用。
