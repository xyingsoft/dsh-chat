...
最后更新：2026-08-30 · 27 个 PR 已合并 · 506 个测试
...
**§43 的骨架步骤与 §44.1.2 的验收清单已全部覆盖。** 阶段 10 标为 🔶：组件与 slot 注册在单元级通过，但客户端插件尚未被真实 DSH 装载 —— 见文末「已知未完成项」。
...
> 组件已就绪但**尚未接入数据源** —— 状态面板的能力表如实标为 `partial` 而不是 `ready`。接数据源需要 host 侧的会话列表查询端点（按会话聚合最后一条消息与未读数），那不在 `P0-a` 的骨架步骤里，见下方「已知未完成项」。
>
...
>
> **但 slot 注册目前只在单元测试里通过，客户端插件从未被真实 DSH 装载。** 这一条此前被标成完成，是不准确的 —— 详见下方「已知未完成项」。
...
| 项 | 现状 | 影响 |
|---|---|---|
| 会话列表的数据源 | 组件与呈现规则已就绪并测过，但 host 没有「按会话聚合最后一条消息与未读数」的查询端点 | 界面能渲染，但拿不到真实数据。能力表如实标为 `partial` |
| relay 客户端抽象 | 三进程验收里 host 直接调 relay 的 HTTP 接口 | 架构分层比文档描述的少一层；跨进程投递本身已验证 |
| **客户端插件未被真实 DSH 装载** | bundle 的 patch 里只有 `@dsh-chat/host`，且渲染侧入口必须是预打包单文件 | DSH 界面上看不到 dsh-chat 的任何 UI |

前两项不在 §43 的骨架步骤与 §44.1.2 的验收清单内，因此不阻塞 `P0-a` 关口。

第三项是**此前记录不准确**：阶段 10 把「slot 注册」标为完成，但那只是单元测试通过。
本地验证时发现两件事：

1. bundle 的 `cordis.patch.yml` 里只有 `@dsh-chat/host`，没有任何包声明 `dsh.client`
   指向客户端插件 —— DSH 根本不知道它存在。已补上 `./client` 子路径导出与转发模块。
2. 渲染侧入口**必须是预打包的单文件**：DSH 的客户端 bundle 是闭包工厂，
   `.module.css` 由 lightningcss 在打包时编译成类名映射加运行时注入的 `<style>`。
   `tsc` 原始产物里那句 `import styles from './X.module.css'` 在渲染进程里会
   直接 `ERR_MODULE_NOT_FOUND`。

上游的构建预设是 monorepo 内的相对导入、没有发布；`dshmarket` 这类外部插件是自己
用 tsdown + lightningcss 重新实现了那套约定。**在做到之前不声明 `dsh.client`** ——
