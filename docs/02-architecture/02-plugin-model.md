...
| | |
|---|---|
| **回答什么问题** | 业务能力如何切分为 Cordis 插件、服务定义与服务提供者与消费者三种角色如何协作，以及 bundle 在 DSH 中以什么形态装载。 |
| **谁该读** | 架构师，插件作者，bundle 维护者，部署管理员 |
| **前置阅读** | [三层总体架构](./01-overall-architecture.md)，[定位与边界](../01-requirements/01-positioning-and-boundaries.md) |

**关键约束**（最易被违反的三条，不能替代正文）：

1. 消费者只能通过服务接口调用能力，不得导入某个提供者的数据库模型，也不得跨插件共享可变内存。
2. 缺少必需提供者时 profile 加载失败，不允许静默降级；未实现的可选能力必须显式呈现为未安装。
3. bundle 只排列插件、提供默认配置并选择提供者，不承载业务单例；其声明的 DSH 运行时依赖必须是精确版本。

> ⚠️ **本篇含契约区**：§6.1 能力矩阵中的领域事件名被 `contract/src/commands.host.spec.ts` 逐个校验，改动需同步代码。
